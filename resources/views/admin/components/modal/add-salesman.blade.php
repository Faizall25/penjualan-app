        ...
        <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
            <div x-show="open" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-xl my-8">
                <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                    <h5 class="font-bold text-lg">Add Salesman</h5>
                </div>
                <div class="p-5">
                    <form action="{{ route('salesman.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="relative mb-4">
                            ...
                            <input name="commission" type="text" placeholder="$$$" class="form-input ltr:pl-10 rtl:pr-10" required />
                        </div>
                        <button type="submit" class="btn btn-primary w-full">Add Salesman</button>
                    </form>
                </div>
            </div>
        </div>
