...
<div class="mt-4 w-full bg-white shadow-[4px_6px_10px_-3px_#bfc9d4] rounded border border-[#e0e6ed] dark:border-[#1b2e4b] dark:bg-[#191e3a] dark:shadow-none p-5">
    <div class="flex justify-between mb-5 items-center">
        <h6 class="text-[#0e1726] font-semibold text-base dark:text-white-light">Salesman Management</h6>
        <div class="flex items-center">
            <form action="{{ route('salesman.index') }}" method="get" class="w-full sm:w-3/4">
                <div class="relative">
            ...
            <!-- modal add -->
            @include('admin.components.modal.add-salesman')
        </div>
    </div>
                                ...
                                <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-show="open" x-transition x-transition.duration.300 class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                                        <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                                            <h5 class="font-bold text-lg">Salesman Admin</h5>
                                        </div>
                                        <div class="p-5">
                                            <form action="{{ route('salesman.update') }}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                @method('put')
